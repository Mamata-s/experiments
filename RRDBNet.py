...
downsample_model = images['lr_4']
input_edges = input_model- downsample_model
label_edges = label-input_model
...
for i in range(epochs):
    ...
    result = net(input_edges)
    output = result['out']
    final_output = input_model+output
    if i==0:
    ...
    if i % n_freq == 0:
        ...
        input = convert_image(input_edges)
        ...
        wandb.log({"fea3": fea3_image})

        trun_image = wandb.Image(trun, caption="trun feature map")      
        wandb.log({"trun_image": trun_image})

        output1= wandb.Image(output1, caption="output")      
        wandb.log({"output": output1})
        # torch.save(net, name+'_'+str(i)+'.pth')
